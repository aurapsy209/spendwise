...
export function ExpenseForm({
  ...
  const { userCategories, addUserCategory } = useAppContext();
  ...
  // Compute the select's displayed value
  const selectValue =
  ...
  const handleSaveCustomCategory = async () => {
    ...
    if (name.length > 40) {
      setCreateError('Category name cannot exceed 40 characters');
      return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      customCategory: formData.categoryId === 'other' && formData.customCategory
